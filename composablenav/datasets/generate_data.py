...
from composablenav.misc.common import load_data, save_json_data
...
def generate_walk_over_env(cfg: DictConfig, folder_name: str, training_idx: int):
...
def main(cfg: DictConfig):
    ...
    start_time = time.time()
    Parallel(n_jobs=cfg.generation.num_proc)(delayed(generate_walk_over_env)(cfg, folder_name, training_idx) for training_idx in range(num_dynamic + num_avoid, num_dynamic + num_avoid + num_prefer))  
    print(f"Prefer Region Env Generation Completed. Time taken: {time.time() - start_time}")

    print(f"All Data Generation Completed at {output_folder}")
