...
def construct_obs_from_yaml(cfg, fn):
    data = load_data(fn)
    obs = data["obs"]
    obstacle_radius = cfg["dynamic_obstacle"]["obstacle_radius"]    
    for info in obs["obstacles_info"]:
        info["radius"] = obstacle_radius
    obstacles_list = [construct_obstacle_from_info(info) for info in obs["obstacles_info"]] if obs.get("obstacles_info") else []
    terrain_list = [construct_obstacle_from_info(info) for info in obs["terrain_info"]] if obs.get("terrain_info") else []
    
    grid_size = data["info"]["grid_size"]
    goal_radius = data["info"]["goal_radius"]
    dt = data["info"]["env_dt"]
    goal_pos = np.array(data["trajectories"]["0"]["goal"]) * (grid_size // 2) # all trajectories share the same goal within the same file
    return obstacles_list, terrain_list, grid_size, goal_pos, goal_radius, dt

def get_paths_from_yaml(fn):
    data = load_data(fn)
    grid_size = data["info"]["grid_size"]
    paths = []
    for traj in data["trajectories"].values():
        path = np.array(traj["path"]) * (grid_size // 2)
...
def validate_plan(path: Union[list, np.ndarray], obstacles, dt, start_time_idx=0, robot_radius=0):
    ...
    for t_idx in range(len(path)-1):
        ...
        for obstacle in obstacles:
            if obstacle.collision(curr_x, curr_y, next_x, next_y, t + start_time, robot_radius) or \
               obstacle.collision(curr_x, curr_y, next_x, next_y, t_plus1 + start_time, robot_radius):
                return False
    return True
...
def forward_motion_rollout_simplified(v, w, x, y, theta, planning_dt):
    ...
    theta_new = theta + w * planning_dt
    theta_estimate = theta + w * planning_dt * 0.5
