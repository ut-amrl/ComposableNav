...
def collision_criteria(path, goal, goal_radius, obstacles, dt, start_time_idx=0, robot_radius=0, **kwargs) -> bool:
    ...
    valid_plan = validate_plan(path, obstacles, dt, start_time_idx=start_time_idx, robot_radius=robot_radius)
    return valid_plan
...
def pass_boolean_criteria(path, obstacles, dt, pass_from_direction: str, **kwargs) -> bool:
    direction = pass_from_direction_criteria(path, obstacles, dt)
    if direction != pass_from_direction:
        return False
    return True
...
def yield_criteria(path, obstacles: list, dt: float, obstacle_width: float, 
    ...
    for idx, step in enumerate(path):
        ...
        pass_from_from_behind = point_in_polygon((x, y), start_polygon)
        if pass_from_from_behind:
            return 0
...
## -------------------------------------------------------------- ##
## ----------------- Critic Functions: pass_from ----------------- ##
## -------------------------------------------------------------- ##
def pass_from_consecutive_criteria(path, obstacles: list, dt: float, n_consecutive_pass_fromn: int):
    assert len(obstacles) == 1
    assert len(path) > 0
    obstacle = obstacles[0]
    current_pass_from_count = 0  # counts the current streak of consecutive pass_froms
    max_consecutive_pass_from_count = 0  # tracks the longest consecutive pass_from streak
    extended_path = path + [path[-1]] * n_consecutive_pass_fromn  # extend path to check full sequence
    ...
    for idx, step in enumerate(extended_path):
        ...
        if x > obs_x: 
            current_pass_from_count += 1
            max_consecutive_pass_from_count = max(max_consecutive_pass_from_count, current_pass_from_count)
        else:
            current_pass_from_count = 0  # reset streak if the path is no longer ahead

    return max_consecutive_pass_from_count


def pass_from_direction_criteria(path, obstacles: list, dt: float):
    assert len(obstacles) == 1
    assert len(path) > 0
    obstacle = obstacles[0]
    direction = None
...
def pass_from_reward(path, obstacles, goal, dt: float, goal_radius:float, pass_from_direction: str):
    # for PPO, return 1 if path is better, 0 otherwise
    assert pass_from_direction in ["left", "right"]
...
def walk_over_reward(path, obstacles, goal, dt: float, goal_radius:float):
    first_waypoint_idx = find_first_waypoint_within_radius(path, goal, goal_radius)
    if first_waypoint_idx != -1:
        path = path[:first_waypoint_idx+1]
    reached_goal = goal_reaching_criteria(path, goal, goal_radius)
...
def pass_from_eval(path1, path2, obstacle, pass_from_direction: str):
    ...
    has_pass_fromn1, direction1 = pass_from_criteria(path1, [obstacle])
    has_pass_fromn2, direction2 = pass_from_criteria(path2, [obstacle])
    if direction1 == direction2:
        if has_pass_fromn1 != has_pass_fromn2:
            return 1 if has_pass_fromn1 else 2
        return 0
    else:
        if direction1 == pass_from_direction:
            return 1
        elif direction2 == pass_from_direction:
            return 2
        return 0
