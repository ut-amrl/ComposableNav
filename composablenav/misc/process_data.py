...
from composablenav.misc.common import construct_obstacle_from_info, pos_in_bounds, load_data, load_fns_from_folder, split_data
...
def construct_normalized_walk_over(prefer_obs, grid_size, mult_x=1, mult_y=1, offset_x=-10, offset_y=0):
    result = []
...
def create_meta_file(saved_data_folder, train_split_ratio, verbose=True):
    filenames = load_fns_from_folder(saved_data_folder, format="json", sort=True)
    train_fns, val_fns = split_data(filenames, train_split_ratio, seed=42)

    def build_metadata(fns):
        indexes, total = [], 0
        for i, fn in  tqdm(enumerate(fns), total=len(fns), desc="Loading trajectories"):
